      ...
      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {dropdown.cards.map((card, index) => (
            <div key={card.slug} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              ...
              <div className="mb-2 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                <img src={card.image || undefined} alt={card.slug} className="aspect-[4/3] w-full object-cover" />
              </div>
              <div className="grid gap-2">
                <label className="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
                  <span className="text-xs font-medium text-slate-700">Show subcategories</span>
                  <button
                    type="button"
                    onClick={() =>
                      setDropdown((prev) => {
                ...
                <label className="space-y-1">
                  <span className="text-xs font-medium text-slate-600">CTA Label</span>
                  <input
                    value={card.ctaLabel}
                    onChange={(event) =>
                      setDropdown((prev) => {
                ...
                <label className="space-y-1">
                  <span className="text-xs font-medium text-slate-600">CTA Href</span>
                  <input
                    value={card.ctaHref}
                    onChange={(event) =>
                      setDropdown((prev) => {
                ...
                <div className="rounded-md border border-slate-200 bg-slate-50 p-2">
                  <div className="mb-2 flex items-center justify-between">
                    <p className="text-xs font-semibold text-slate-700">Subcategory Visibility</p>
                  ...
                  {loadingBySlug[card.slug] ? (
                    <p className="text-xs text-slate-500">Loading subcategories...</p>
                  ) : subCategoryCache[card.slug]?.length ? (
                    <div className="space-y-1.5">
                      {subCategoryCache[card.slug].map((sub) => {
                        const isHidden = card.hiddenSubcategorySlugs.includes(sub.slug);
                        return (
                          <div key={`${card.slug}-${sub.slug}`} className="flex items-center justify-between gap-2 rounded-md border border-slate-200 bg-white px-2 py-1.5">
                            <span className="truncate text-xs text-slate-700">{sub.name}</span>
                            <button
                              type="button"
                              onClick={() =>
                                setDropdown((prev) => {

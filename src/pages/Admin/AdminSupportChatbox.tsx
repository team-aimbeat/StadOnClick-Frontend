...
import { getSupportSocket, initSupportSocket } from "@/lib/supportSocket";
...
export default function AdminSupportChatbox() {
  ...
  useEffect(() => {
    ...
    const handler = (payload: any) => {
      toast.custom(
        <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-lg">
          <p className="font-semibold text-slate-900">New support activity</p>
          <p className="text-xs text-slate-600">
            {payload?.message?.body
              ? payload.message.body.slice(0, 80)
              : payload?.ticketNumber
              ? `Ticket ${payload.ticketNumber} created`
              : "New update"}
          </p>
        </div>,
    ...
    socket?.on("support:ticket.created", handler);
    socket?.on("support:message.created", handler);
    return () => {
      socket?.off("support:ticket.created", handler);
  ...
  return (
    <div className="space-y-4 px-2 sm:px-4">
      ...
      <div className="flex h-[calc(100vh-200px)] overflow-hidden rounded-2xl border bg-white shadow-sm">
        <aside className="w-[360px] shrink-0 border-r bg-slate-50/70">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-b bg-white/70">
            <Select
              value={status ?? "ALL"}
        ...
        <main className="flex flex-1 flex-col bg-[#efeae2]">
          <header className="flex items-center justify-between gap-3 border-b bg-white/90 px-4 py-3 backdrop-blur shadow-sm">
          ...
          <div className="sticky bottom-0 border-t bg-white/95 px-3 py-2 backdrop-blur">
            ...
            {!canSend && ticket ? (
              <p className="mt-1 text-xs text-amber-600">
                Assigned to another agent. Only admins/support admins can override.
              </p>
            ) : null}
          </div>
        </main>

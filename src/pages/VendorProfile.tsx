...
const stripCoordsTag = (value: string) =>
  value
    .replace(
      /\s*\[\s*coords\s*:\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*\]\s*$/i,
      "",
    )
    .trim();
...
const VendorProfile = () => {
  ...
  const shouldSkipVendorProfileQuery =
    isBusinessOnboardingRoute && !isVendorUser;
  ...
  const {
    ...
    skip: shouldSkipVendorProfileQuery,
  });
  ...
  const [updateProfile, { isLoading: isUpdating }] =
    useUpdateVendorProfileMutation();
  const [createBusinessProfile, { isLoading: isCreating }] =
    useCreateVendorBusinessProfileMutation();
  ...
  const validateBusinessHour = (slot: BusinessHour) =>
    (slot.day?.trim().length ?? 0) >= 2 &&
    (slot.value?.trim().length ?? 0) >= 2;
  ...
  const parseBusinessHourValue = (value: string) => {
    const [rawStart = "", rawEnd = ""] = value.includes(" - ")
      ? value.split(" - ")
      : value.split(/\s*-\s*/);
  ...
  const updateBusinessHourTimeRange = (
    ...
    const range =
      startText || endText ? `${startText} - ${endText}`.trim() : "";
    updateBusinessHour(index, "value", range);
  };
  const resolveDayKey = (value: string) => {
    const normalized = value.trim().toLowerCase();
    const match = weekdayOptions.find((day) =>
      normalized.startsWith(day.toLowerCase()),
    );
    return match ?? "Mon";
  };
  const businessHourSlotsByDay = useMemo(() => {
    const grouped: Record<
      string,
      Array<{ index: number; slot: BusinessHour }>
    > = {
      Mon: [],
      Tue: [],
      Wed: [],
  ...
  const handleSave = async () => {
    ...
    const invalidIndexes = businessHours.reduce<number[]>(
      (errs, slot, index) => {
    ...
    if (invalidIndexes.length > 0) {
      ...
      toast.error(
        "Each business hour entry needs at least 2 characters for day and time.",
      );
      return;
    }
    const headquartersForSave = buildHeadquartersValue(
      headquarters,
      headquartersCoords,
    );
    try {
      ...
      toast.error(
        error?.data?.message || error?.data?.error || "Failed to save profile",
      );
    }
  };
    ...
    return [
      {
        ...
        done: Boolean(profile.description?.trim()),
      },
      {
        label: "Upload KYC documents",
        done: profile.kycStatus !== "NOT_SUBMITTED",
      },
        ...
        {profileError && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
            ...
            <p className="text-sm text-amber-700 mt-0.5">
              Fill the fields below and save to create your business profile and
              generate vendor ID.
            </p>
          </div>
        )}
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
                ...
                >
                  ...
                  >
                    <path
                      ...
                      strokeWidth={2.5}
                      d="M11 5h2m-1 0v14m0 0l4-4m-4 4l-4-4"
                    />
                  </svg>
                </button>
              </div>
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 max-w-2xl text-sm font-medium leading-6 text-slate-500">
                  Complete your profile to start receiving bookings from new
                  clients.
                </p>
            ...
            <div className="flex flex-wrap items-center gap-3 lg:justify-end">
              <button
                type="button"
                onClick={() =>
                  navigate(`/vendor/${profile?.slug || "profile"}`)
                }
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M15 12H3m0 0l4-4m-4 4l4 4m13-8v8a2 2 0 01-2 2H6"
                  />
                </svg>
                View Public Profile
              </button>
              <button
                ...
                onClick={handleSave}
                disabled={isSaving}
                className="inline-flex items-center gap-2 rounded-xl bg-[#4F7DFF]  px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#2742be] disabled:cursor-not-allowed disabled:bg-blue-400"
              >
                  ...
                  <div className="grid h-11 w-11 place-items-center rounded-full bg-blue-50 text-blue-600">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M12 11c1.657 0 3-1.79 3-4s-1.343-4-3-4-3 1.79-3 4 1.343 4 3 4zM6 20a6 6 0 0112 0"
                      />
                    </svg>
                  </div>
                  <div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm leading-6 text-slate-600">
                            Your business profile is the first thing potential
                            clients see. Providing accurate and detailed
                            information increases trust and helps our matching
                            algorithm place your services in front of the right
                            buyers. Vendors with complete profiles see a 40%
                            increase in initial inquiries.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-2 text-sm text-slate-500">
                          Optimise how your business appears in search engine
                          results.
                        </p>
                      </div>
                      <button
                        ...
                        onClick={handleSave}
                        disabled={isSaving}
                        className="inline-flex items-center rounded-xl bg-[#4F7DFF]  px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#2742be] disabled:cursor-not-allowed disabled:bg-blue-400"
                      >
                        {isSaving ? "Saving..." : "Save Changes"}
                      </button>
                    </div>
                          ...
                          <div className="mt-5 flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4">
                            ...
                            <button
                              ...
                              className={`relative inline-flex h-7 w-12 shrink-0 items-center rounded-full transition-colors ${
                                isIndexable ? "bg-[#4F7DFF] " : "bg-slate-200"
                              }`}
                            >
                              <span
                        ...
                        <div className="rounded-[24px] border border-slate-100 bg-white p-5 shadow-[0_18px_50px_-40px_rgba(15,23,42,0.22)]">
                          ...
                          <div className="mt-4 flex items-center gap-4">
                            ...
                            <p className="text-sm leading-6 text-slate-600">
                              Your meta description is the perfect length.
                              Consider adding 2 more high-intent keywords to
                              improve ranking.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="rounded-[24px] border border-slate-100 bg-white p-5 shadow-[0_18px_50px_-40px_rgba(15,23,42,0.22)]">
                          ...
                          <ul className="mt-4 space-y-4 text-sm text-slate-600">
                            <li className="flex items-start gap-3">
                              ...
                              <span>
                                Include your primary business category in the
                                title tag.
                              </span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="mt-0.5 text-[#3554e0]">●</span>
                              <span>
                                Keep descriptions engaging to improve
                            ...
                            <li className="flex items-start gap-3">
                              <span className="mt-0.5 text-[#3554e0]">●</span>
                              <span>
                                Ensure your business location is mentioned in
                          ...
                          <div className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-blue-600">
                            ...
                            >
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8m-18 8h18a1 1 0 001-1V7a1 1 0 00-1-1H3a1 1 0 00-1 1v8a1 1 0 001 1z"
                              />
                            </svg>
                          </div>
                          <div>
                          ...
                          <div className="mt-0.5 grid h-8 w-8 place-items-center rounded-xl bg-blue-100 text-blue-600">
                            ...
                            >
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M12 8v4m0 4h.01M4.93 19h14.14c1.54 0 2.51-1.67 1.74-3L14.74 4.99c-.77-1.33-2.71-1.33-3.48 0L3.19 16c-.77 1.33.2 3 1.74 3z"
                              />
                            </svg>
                          </div>
                          <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-slate-600">
                              Your location data is synchronized with public
                              business registries for authenticity.
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                              Search, drop a pin, or reuse the saved office
                              address.
                            </p>
                          </div>
                          <div className="flex items-center gap-2 rounded-full bg-slate-50 p-1 text-xs font-semibold text-slate-500">
                            ...
                            <span className="rounded-full bg-[#edf2ff] px-3 py-1 text-[#3554e0]">
                              Drop pin here
                            </span>
                          </div>
                        </div>
                          ...
                          <div className="mt-4 grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
                            <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3">
                              ...
                              <p className="mt-2 text-sm font-semibold text-slate-800">
                                {headquartersCoords.lat !== null
                                  ? `${headquartersCoords.lat.toFixed(4)}Â° N`
                                  : "Not set"}
                              </p>
                            </div>
                            <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3">
                              ...
                              <p className="mt-2 text-sm font-semibold text-slate-800">
                                {headquartersCoords.lng !== null
                                  ? `${headquartersCoords.lng.toFixed(4)}Â° E`
                                  : "Not set"}
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="rounded-[18px] border border-slate-100 bg-white p-4 ">
                          ...
                          <p className="mt-2 text-sm leading-6 text-slate-600">
                            Changes are applied immediately to your public
                            profile.
                          </p>
                        </div>
                        <div className="rounded-[18px] border border-slate-100 bg-white p-4 ">
                          ...
                          <p className="mt-2 text-sm leading-6 text-slate-600">
                            Keep the contact and location information current
                            for better trust.
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="space-y-5">
                    <div className="rounded-[24px] border border-slate-100 bg-white px-6 py-5 ">
                      ...
                      <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                        Set your availability for clients and automated booking
                        sequences. Changes here reflect across your storefront
                        and service listings in real time.
                      </p>
                    </div>
                      ...
                      <div className="rounded-[24px] border border-slate-100 bg-white p-5">
                        ...
                        <div className="mt-4 space-y-2">
                          {weekdayOptions.map((day) => {
                            const count =
                              businessHourSlotsByDay[day]?.length ?? 0;
                            ...
                            return (
                              <button
                                ...
                                className={`flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left transition ${
                                  isActive
                                    ? "bg-[#4F7DFF]  text-white shadow-lg shadow-blue-200"
                        ...
                        <div className="rounded-[24px] border border-slate-100 bg-white p-5">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                type="button"
                                className="relative inline-flex h-6 w-11 items-center rounded-full bg-[#4F7DFF] "
                            ...
                            <div className="mt-5 space-y-4">
                              {selectedDaySlots.length > 0 ? (
                                selectedDaySlots.map(({ index, slot }) => {
                                  const rowInvalid =
                                    invalidHours.includes(index);
                                  const { startTime, endTime } =
                                    parseBusinessHourValue(slot.value ?? "");
                                  return (
                                    <div
                                      key={index}
                                    ...
                                    >
                                      ...
                                      {rowInvalid && (
                                        <p className="mt-2 text-xs text-red-500">
                                          Day and time must each be at least 2
                                          characters.
                                        </p>
                                      )}
                                    </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                          {[
                            ...
                            <div
                              key={card.title}
                              className="rounded-[20px] border border-slate-100 bg-white p-4 shadow-[0_18px_50px_-40px_rgba(15,23,42,0.16)]"
                            >
                              <p className="text-sm font-bold text-slate-900">
                                {card.title}
                              </p>
                        ...
                        <div className="overflow-hidden rounded-[24px] border border-slate-100 bg-white shadow-[0_18px_50px_-38px_rgba(15,23,42,0.18)]">
                          <div className="h-48 bg-[linear-gradient(135deg,#2f7f74_0%,#3aa9a0_100%)] p-4">
                            <div className="flex h-full items-center justify-center rounded-[18px] border border-white/20 bg-white/10 backdrop-blur-sm">
                              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#4F7DFF]  text-white shadow-lg">
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  className="h-6 w-6"
                                  fill="none"
                  ...
                  <div className="flex items-center gap-2 pt-4">
                    ...
                    {!isLastStep && (
                      <button
                        type="button"
                        onClick={goNextStep}
                        className="rounded-xl px-5 py-2.5 text-sm font-semibold text-blue-600 hover:bg-blue-50 transition"
                      >
                        Next: {nextStepLabel}
                      </button>
                    )}

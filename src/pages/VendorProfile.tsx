  ...
  return (
    <DashboardContainer className="pb-24">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {/* ── Blue Cover Banner ── */}
        <div className="relative h-44 rounded-2xl overflow-hidden bg-gradient-to-r from-blue-700 via-blue-500 to-blue-400">
          {/* Geometric decorations */}
          <div className="absolute -top-10 -right-10 w-52 h-52 rounded-full bg-white/10" />
          <div className="absolute top-4 right-28 w-32 h-32 rotate-12 rounded-2xl bg-white/10" />
          <div className="absolute -bottom-8 right-56 w-28 h-28 rotate-45 rounded-xl bg-white/10" />
          <div className="absolute top-8 right-8 w-16 h-16 rounded-xl bg-white/10" />

          {/* Change Cover button */}
          <button className="absolute top-4 right-4 flex items-center gap-2 rounded-lg border border-white/30 bg-white/20 px-4 py-2 text-sm font-medium text-white backdrop-blur-sm hover:bg-white/30 transition">
                  ...
                  <div className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} className={inputCls} placeholder="Your business name" />
                      </div>
                      <div>
                        <label className={labelCls}>City ID</label>
                        <input type="text" value={cityId} onChange={(e) => setCityId(e.target.value)} className={inputCls} placeholder="UUID" />
                      </div>
                    </div>
                    <div>
                      <label className={labelCls}>Description</label>
                      <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className={`${inputCls} resize-y`} placeholder="Describe your business..." />
                    </div>
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label className={labelCls}>Headquarters</label>
                        <input type="text" value={headquarters} onChange={(e) => setHeadquarters(e.target.value)} className={inputCls} placeholder="City, Country" />
                      </div>
                      <div>
                        <label className={labelCls}>Services Focus</label>
                        <input type="text" value={serviceOverview} onChange={(e) => setServiceOverview(e.target.value)} className={inputCls} placeholder="e.g. Cleaning, Repairs..." />
                      </div>
                    </div>
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-5 py-4">
                      <p className="text-sm font-semibold text-slate-700">Why this information matters</p>
                      <p className="text-sm text-slate-500 mt-1">Clear details help customers find and trust your business faster.</p>
                    </div>
                  </div>
                )}
